		...
		void BeginSession(const std::string& name, const std::string& filepath = "results.json")
		{
			...
			if (m_CurrentSession) {
				...
				if (Log::GetCoreLogger()) { // Edge case: BeginSession() might be before Log::Init()
					PA_CORE_ERROR("Instrumentor::BeginSession('{0}') when session '{1}' already open.", name, m_CurrentSession->Name);
				}
				InternalEndSession();
			}
			...
			if (m_OutputStream.is_open()) {
				...
				if (Log::GetCoreLogger()) { // Edge case: BeginSession() might be before Log::Init()
					PA_CORE_ERROR("Instrumentor could not open results file '{0}'.", filepath);
				}
			}
		}
...
#define PA_PROFILE 0
	...
	#define PA_PROFILE_SCOPE(name) ::Pistachio::InstrumentationTimer timer##__LINE__(name);
	#define PA_PROFILE_FUNCTION() PA_PROFILE_SCOPE(PA_FUNC_SIG)
#else
	#define PA_PROFILE_BEGIN_SESSION(name, filepath)
	#define PA_PROFILE_END_SESSION()
